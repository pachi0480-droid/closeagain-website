...
export function CapabilityDetail({
  ...
  return (
    <section
      id={capability.id}
      className={`grain relative scroll-mt-24 py-16 md:py-20 ${
          ...
          <div className="lg:grid lg:grid-cols-12 lg:gap-x-12">
            <div className="lg:col-span-4">
              <div className="flex items-baseline gap-4">
                ...
                <h2 className="text-h3 text-graphite">{capability.name}</h2>
              </div>
              <p className="mt-3 pl-9 font-mono text-mono-xs text-graphite-3 uppercase">
                {capability.stage}
              </p>
              <p className="mt-6 max-w-[38ch] pl-9 text-[1.0625rem] leading-relaxed text-graphite-2">
                {capability.body}
              </p>
            </div>
            ...
            <div className="mt-10 lg:col-span-7 lg:col-start-6 lg:mt-0">
              <dl className="border-t border-rule">
                {rows.map((row) => (
                  <div
                    key={row.label}
                    className="grid gap-x-8 gap-y-1.5 border-b border-rule py-5 sm:grid-cols-[9rem_1fr]"
                  >
                    <dt className="font-mono text-mono-xs text-graphite-3 uppercase">
                      {row.label}
                    </dt>
                    <dd className="max-w-[54ch] text-[0.9375rem] leading-relaxed text-graphite-2">
                      {row.value}
                    </dd>
                  </div>
                ))}
              ...
              <div className="mt-6 flex gap-3">
                <span aria-hidden="true" className="mt-2.5 h-px w-6 shrink-0 bg-recover" />
                <p className="max-w-[46ch] text-[0.9375rem] leading-relaxed text-graphite">
