    ...
    <table className="w-full border-collapse text-left">
      ...
      <thead>
        <tr className="border-b border-rule">
          <th scope="col" className="py-3 font-mono text-mono-xs font-normal text-graphite-3 uppercase">
            If you recover
          </th>
          <th scope="col" className="py-3 text-right font-mono text-mono-xs font-normal text-graphite-3 uppercase">
            Per month
          </th>
          <th scope="col" className="hidden py-3 text-right font-mono text-mono-xs font-normal text-graphite-3 uppercase sm:table-cell">
            Per year
          </th>
        </tr>
      </thead>
      <tbody>
        {rows.map((row) => (
          <tr key={row.rate} className="group border-b border-rule-soft">
            <th scope="row" className="py-4 pr-4 font-normal">
              <span className="flex items-center gap-3">
                <span
                  className={`tnum font-mono text-[0.9375rem] ${
                    row.current ? 'text-recover' : 'text-graphite-2'
                  }`}
                >
                  {row.rate}%
                </span>
            ...
            <td
              className={`tnum py-4 text-right align-top font-mono text-[0.9375rem] ${
                row.current ? 'text-graphite' : 'text-graphite-2'
              }`}
            >
              {formatCurrency(row.monthlyValue)}
            </td>
            <td className="tnum hidden py-4 text-right align-top font-mono text-[0.9375rem] text-graphite-3 sm:table-cell">
