      ...
      <div className="shell">
        <div className="lg:grid lg:grid-cols-12 lg:gap-x-12">
          ...
          <div className="mt-8 lg:col-span-6 lg:col-start-7 lg:mt-2">
            <Reveal delay={140}>
              <p className="max-w-[48ch] text-lede text-chalk-2">
        ...
        <Reveal delay={100}>
          <ul className="mt-14 grid border-t border-rule-ink md:mt-16 sm:grid-cols-2">
            {integrationTargets.map((target, i) => (
              <li
                key={target.category}
                className={`border-b border-rule-ink py-7 ${
                  i % 2 === 1 ? 'sm:border-l sm:pl-10' : 'sm:pr-10'
                }`}
              >
                <div className="flex items-baseline justify-between gap-4">
                  <h3 className="text-[1.0625rem] tracking-[-0.015em] text-chalk">
                    {target.category}
                  </h3>
                  <span className="shrink-0 rounded-[5px] border border-rule-ink px-2 py-1 font-mono text-mono-xs text-chalk-3 uppercase">
                    Planned
                  </span>
                </div>
                <p className="mt-2.5 max-w-[42ch] text-[0.9375rem] leading-relaxed text-chalk-2">
                  {target.note}
                </p>
              </li>
            ))}
