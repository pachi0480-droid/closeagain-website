...
export function AgainstPlans({ result }: { result: CalculatorResult }) {
  ...
  return (
    <ul className="border-t border-rule-ink">
      {priced.map((plan) => {
        ...
        return (
          <li
            key={plan.id}
            className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 border-b border-rule-ink py-4"
          >
            <span className="flex items-baseline gap-3">
              <Link
                href="/pricing"
                className="link-rule text-[1.0625rem] text-chalk"
              >
                {plan.name}
              </Link>
